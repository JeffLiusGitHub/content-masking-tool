...
|---|---|---|---|
| WIN-001 | package layout | a real WiX 4 per-machine MSI installs the complete PyInstaller onedir tree, including `_internal`, beneath the stable Program Files product directory | Passed for unsigned test MSI — run 34936179341 |
...
| WIN-003 | silent install | `msiexec /i package.msi /qn /norestart /log install.log` succeeds as SYSTEM using standard MSI exit codes; any possible 3010 handling is documented, no restart is required where avoidable, and version/frozen smoke probes pass afterward | Passed for unsigned test MSI — run 34936179341 |
| WIN-004 | repeat and upgrade | same-version deployment is idempotent; a prior MSI performs one in-place Major Upgrade; old and new copies do not coexist | Partial — same-version repeat passed; cross-version Major Upgrade pending |
...
| WIN-006 | managed uninstall | `msiexec /x {ProductCode} /qn /norestart /log uninstall.log` removes only installer-owned program files and registration | Passed for unsigned test MSI — run 34936179341 |
| WIN-007 | data retention and context | install, repair, upgrade, and ordinary uninstall as SYSTEM preserve user-profile sentinels for `%APPDATA%\ContentMaskingTool\` Vaults, deny lists, settings, history, reviews, and audit data; no application state is redirected into SYSTEM or another user's profile | Partial — install/reinstall/uninstall sentinel passed; repair and upgrade pending |
...
| ID | Planned verification | Acceptance | Status |
|---|---|---|---|
| MAC-001 | package layout | a real PKG installs the complete onedir payload into the documented machine-level directory and installs only the intended secure launcher/entry point | Passed for unsigned test PKGs on arm64/x86_64 — run 34936179341 |
| MAC-002 | architecture | installed Mach-O files match the asset architecture; Intel support is not silently dropped | Passed for unsigned test PKGs on arm64/x86_64 — run 34936179341 |
| MAC-003 | silent install | `sudo installer -pkg package.pkg -target /` succeeds noninteractively with expected root ownership and file modes | Passed for unsigned test PKGs on arm64/x86_64 — run 34936179341 |
| MAC-004 | receipt and version | `pkgutil --pkg-info` reports the stable package identifier and release version; the application version probe and minimal smoke test pass | Passed for unsigned test PKGs on arm64/x86_64 — run 34936179341 |
| MAC-005 | repeat and upgrade | repeated installation is idempotent and a prior PKG upgrades without removing per-user data | Partial — same-version repeat passed on arm64/x86_64; cross-version upgrade pending |
| MAC-006 | managed uninstall | the PKG installs the root-owned mode-0755 helper at `/Library/Application Support/ContentMaskingTool/uninstall.sh`; the manifest records a literal guarded invocation that is idempotent, removes only allow-listed package files/receipt, and preserves user data | Partial — helper lifecycle passed on arm64/x86_64; final release-manifest field pending |
| MAC-007 | data retention | install, upgrade, and managed uninstall preserve `~/Library/Application Support/ContentMaskingTool/` | Partial — install/reinstall/uninstall sentinel passed on arm64/x86_64; cross-version upgrade pending |
